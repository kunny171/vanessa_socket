	...
	void *UNUSED(data))
{
	...
	//Get keyword from mysql and out to arraystring
	MYSQL *conn;
	MYSQL_RES *res;
	MYSQL_ROW row;
	int lengths;
	...
	if (mysql_query(conn, "select keyword from replace_keyword") == 0) {
		...
		if (res != NULL) {
			...
			int i, j;
			for (i = 0; i < lengths; i++) {
				arrayString[i] = (char*)malloc(255 * sizeof(char));
			}
			//copy keyword to arraystring
			int index = 0;
			...
			for(i=0; i < count; i++){
				for(j=0; j < index-1; j++){
					//compare string
					if(strcmp(temp[i],arrayString[j]) == 0){
						//if temp[i] == arrayString[i] => replace 
						strcpy(temp[i], stringReplace);
					}
				}
			}
			
			//sau khi replace xong ep kieu nguoc lai char to byte
			//Ep kieu byte to char
			for (i = 0; i < count; i++) {
				bytes[i] = (ssize_t)temp[i];
			}
		}
		else
			printf("Query failed\n");
